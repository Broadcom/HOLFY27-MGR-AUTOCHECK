...
#   2. Creating the testing flag file to skip git clone/pull in labstartup.sh
#   3. Setting lockholuser = false in config.ini and holodeck/*.ini files
#   4. Removing external URLs from config.ini URLS check list
...
TESTING_FLAG_FILE = f'{LMCHOL_ROOT}/testing'
...
def step_create_testing_flag():
    """Create the testing flag file used by labstartup.sh to skip git ops."""
    print('\n--- Step 2: Create testing flag file ---')

    if os.path.isdir(LMCHOL_ROOT):
        action(f'Creating testing flag: {TESTING_FLAG_FILE}')
        if not _dry_run:
            with open(TESTING_FLAG_FILE, 'w') as f:
                f.write(f'OFFLINE MODE - Set by offline-ready.py at '
                        f'{datetime.datetime.now().isoformat()}\n'
                        f'This file causes labstartup.sh to skip git '
                        f'clone/pull operations.\n')
            os.chmod(TESTING_FLAG_FILE, 0o644)
    else:
        log(f'{LMCHOL_ROOT} not mounted -- cannot create testing flag', 'ERROR')
        log('The console NFS mount must be available. Run mount.sh first '
            'or ensure the console VM is running.', 'ERROR')
        return False
...
def step_remove_external_urls():
    """Remove external URLs from config.ini URLS section."""
    print('\n--- Step 4: Remove external URLs from config.ini ---')

    if not os.path.isfile(CONFIG_INI):
        log(f'{CONFIG_INI} not found, skipping URL cleanup', 'WARN')
        return
    ...
    try:
        ...
        log(f'Error reading {CONFIG_INI}: {e}', 'ERROR')
        return

    # Parse the URLS section manually to preserve formatting
    lines = content.split('\n')
    new_lines = []
    ...
    for line in lines:
        ...
        # Detect start of URLS key
        if re.match(r'^URLS\s*=', stripped):
            in_urls_section = True
            # The first URL is on this line after the =
            ...
            if url_part and not url_part.startswith('#'):
                ...
                if is_internal_url(url_only):
                    ...
                    # Comment out the entire URLS = line with external URL
                    # and start fresh with only internal URLs
                    action(f'Removing external URL: {url_only}')
                    urls_removed.append(url_only)
                    # We need to write the URLS key but skip this value
                    # We'll reconstruct after collecting all URLs
                    new_lines.append(f'URLS = # External URLs removed by offline-ready.py')
            else:
                new_lines.append(line)
            continue

        # Check for continuation lines in URLS section
        if in_urls_section:
            ...
            if stripped and not stripped.startswith('#') and (
                ...
                # Commented-out continuation line, keep it
                new_lines.append(line)
                continue
            else:
                # End of URLS section
    ...
    if urls_removed:
        action(f'Removed {len(urls_removed)} external URL(s), '
               f'kept {len(urls_kept)} internal URL(s)')
        if not _dry_run:
            with open(CONFIG_INI, 'w') as f:
                f.write('\n'.join(new_lines))
    else:
        log('No external URLs found in config.ini')
...
def step_disable_vlp_agent():
    ...
    # The VLP Agent is started by labstartup.sh (line 388-398) based on
    # cloud environment detection. In offline mode, vmtoolsd will return
    # "No value found" which causes it to not start. However, the holuser
    # cron also starts it: /home/holuser/hol/Tools/VLPagent.sh &
    # We create a marker that the cron entry and labstartup.sh can check.
    vlp_disable_marker = '/tmp/vlp-disabled'
    action(f'Creating VLP disable marker: {vlp_disable_marker}')
    if not _dry_run:
        with open(vlp_disable_marker, 'w') as f:
            f.write(f'VLP Agent disabled by offline-ready.py at '
                    f'{datetime.datetime.now().isoformat()}\n')
        os.chmod(vlp_disable_marker, 0o644)

    # Also create a persistent marker in holuser home that survives reboots
    # since /tmp is typically cleared on reboot
    vlp_persistent = f'{HOLUSER_HOLROOT}/.vlp-disabled'
    action(f'Creating persistent VLP disable marker: {vlp_persistent}')
    if not _dry_run:
...
def print_summary():
    ...
    print('  2. Create testing flag file (skip git clone/pull in labstartup.sh)')
    print('  3. Set lockholuser = false in config.ini and holodeck/*.ini')
    print('  4. Remove external URLs from config.ini URL checks')
